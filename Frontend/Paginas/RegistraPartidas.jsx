...
function RegistraPartida({ setMensagem }) {
    ...
    return (<main className="sm:ml-[60px] flex flex-col">
        ...
        {!jogadoresSelecionados ? <form className="gap-5" onSubmit={() => { }}>
            <h1>Jogadores</h1>
                ...
                <div className="flex flex-col">
                    ...
                    {listaBrancos[0] && timeBranco[1] && listaBrancos.length > 0 ? (
                        <div className={`border h-auto flex flex-col`}>
                            {listaBrancos[1].map(e => (<div key={e.ID_jogador || index} className="p-[7px] hover:bg-gray-200" onClick={() => { setBranco([null, e.nicknameJogador]); setListaBrancos([false, listaBrancos]) }}>{e.nicknameJogador}</div>))}
                        </div>)
                        : null}
                </div>
